...
func ShowWordp(word []rune, nbrOfLetterToShow int) ([]string, []string) { // give the letter that are given with the word
	...
	for nbrOfLetterToShow != 0 {
		...
		letterChoosen := string(word[rand.Intn(len(word))])
		for i := 0; i < len(word); i++ {
			...
			if letterAlreadyRolld == 0 {
				...
				for f < len(word) { // pass throught the list and replace the corresponding _ with the letter
					if word[f] == rune(letterChoosen[0]) {
